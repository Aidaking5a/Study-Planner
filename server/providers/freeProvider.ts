import type { Flashcard, QuizQuestion, StudyInput, StudySessionResult } from "../../shared/types.js";
import { languageLabels, levelLabels } from "../agentApi.js";
...
export function createFreeStudySession(input: StudyInput): StudySessionResult {
  ...
  return {
    ...
    studyPlan,
    complianceNote:
      "Free beta output is generated locally with deterministic templates. It is for self-study support only, not official grading, ranking, or school placement."
  };
}

export function runFreeAgent(agentId: string, input: StudyInput): unknown {
  const session = createFreeStudySession(input);
